...
METHOD New( oUrl, lTrace, oCredentials, lSSL, CAFile,CaPath,certfile,keyfile,passfile ) CLASS tIPClientSSL
   LOCAL oErr
...
METHOD Open( cUrl ) CLASS tIPClientSSL
   ...
   if ::lSSL
      ::SocketCon := InetSSLCreate( , ::CAFile, ::CaPath,::certfile ,;
                                                         ::keyfile  ,;
                                                         ::passfile  )
...
METHOD Close() CLASS tIPClientSSL
...
METHOD Reset() CLASS tIPClientSSL
   ::bInitialized := .F.
   ::bEof := .F.
RETURN .T.
...
METHOD Commit() CLASS tIPClientSSL
RETURN .T.



METHOD Read( nLen ) CLASS tIPClientSSL
   LOCAL cStr0, cStr1
...
METHOD ReadToFile( cFile, nMode, nSize ) CLASS tIPClientSSL
   LOCAL nFout
   LOCAL cData
   LOCAL nSent
...
METHOD WriteFromFile( cFile ) CLASS tIPClientSSL
   LOCAL nFin
   LOCAL cData
   LOCAL nLen
...
METHOD Data( cData ) CLASS tIPClientSSL
...
METHOD Write( cData, nLen, bCommit ) CLASS tIPClientSSL
...
METHOD InetSendAll( SocketCon, cData, nLen ) CLASS tIPClientSSL
...
METHOD InetCount( SocketCon ) CLASS tIPClientSSL
...
METHOD InetRecv( SocketCon, cStr1, len ) CLASS tIPClientSSL
...
METHOD InetRecvLine( SocketCon, nLen, size ) CLASS tIPClientSSL
...
METHOD InetRecvAll( SocketCon, cStr1, len ) CLASS tIPClientSSL
...
METHOD InetErrorCode( SocketCon ) CLASS tIPClientSSL
...
METHOD InetErrorDesc( SocketCon ) CLASS tIPClientSSL
   LOCAL cMsg := ""
...
METHOD InetConnect( cServer, nPort, SocketCon ) CLASS tIPClientSSL
...
METHOD InetRcvBufSize( SocketCon, nSizeBuff ) CLASS tIPClientSSL
   IF ! Empty( nSizeBuff )
      INETSSLSETRCVBUFSIZE( SocketCon, nSizeBuff )
   ENDIF
RETURN INETSSLGETRCVBUFSIZE( SocketCon )

METHOD InetSndBufSize( SocketCon, nSizeBuff ) CLASS tIPClientSSL
   IF ! Empty( nSizeBuff )
...
METHOD SetProxy( cProxyHost, nProxyPort, cProxyUser, cProxyPassword )  CLASS tIPClientSSL
