...
CLASS tIPClientSSLHTTP FROM tIPClientSSL
   DATA cMethod
   DATA nReplyCode
   DATA cReplyDescr
...
METHOD New( oUrl,lTrace, oCredentials, CAFile,CaPath,certfile,keyfile,passfile ) CLASS tIPClientSSLHTTP
   local cFile := "httpcln"
   local n := 0

   ::super:new( oUrl, lTrace, oCredentials,, CAFile,CaPath,certfile,keyfile,passfile  )
...
METHOD Get( cQuery ) CLASS tIPClientSSLHTTP
...
METHOD Post( cPostData, cQuery ) CLASS tIPClientSSLHTTP
...
METHOD StandardFields() CLASS tIPClientSSLHTTP
   LOCAL iCount
...
METHOD ReadHeaders(lClear) CLASS tIPClientSSLHTTP
...
METHOD Read( nLen ) CLASS tIPClientSSLHTTP
...
METHOD ReadAll() CLASS tIPClientSSLHTTP
...
METHOD setCookie(cLine) CLASS tIPClientSSLHTTP
...
METHOD getcookies(cHost,cPath) CLASS tIPClientSSLHTTP
...
METHOD Boundary(nType) CLASS tIPClientSSLHTTP
   /*
...
METHOD Attach(cName,cFileName,cType) CLASS tIPClientSSLHTTP
   aadd(::aAttachments,{cName,cFileName,cType})
return(nil)

METHOD PostMultiPart( cPostData, cQuery ) CLASS tIPClientSSLHTTP
   ...
   FOR each oSub in ::aAttachments
      ...
      while nRead=nBuf
         ...
         * 12/07/2012 - 11:11 - Luciano Iuri Pereira
         * Inserido IF para ler arquivos binários (imagens) com o FRead() pois
         * o FReadStr() estava truncando a leitura dos dados.         
         * Implementado para emitir danfe com o logotipo da empresa.
...
METHOD Post2MultiPart( cPostData, cQuery ) CLASS tIPClientSSLHTTP
   LOCAL cData:="", nI, cTmp,y,cBound:=::boundary()
   local cCrlf:=::cCRlf,oSub
   local nPos
   local cFilePath,cName,cFile,cType
...
METHOD WriteAll( cFile ) CLASS tIPClientSSLHTTP
...
PROCEDURE httpClnDestructor CLASS tIPClientSSLHTTP
